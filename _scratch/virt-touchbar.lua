--luacheck: ignore module.eventWatcher
--
...
module.eventwatcher =
    eventtap.new(
    {events.flagsChanged, events.keyDown, events.leftMouseDown},
    function(ev)
        ...
        if ev:getType() == events.flagsChanged and rawFlags == 524352 then
            hs.alert("right opt pressed.")
            -- module.toggle()
            rightOptPressed = true
            module.countDown =
                timer.doAfter(
                module.rightOptPressTime,
                function()
                    if rightOptPressed then
                        module.toggle()
                    end
                end
            )
