...
-- proxy changed method
function hs.mouse.absolutePosition()
...
function get_relative_mouse(mouse, frame)
    ...
    if mouse.x > right then
        rmx = right
    end
    if mouse.y > bottom then
        rmy = bottom
    end
...
function mouseHasMoved(old, new)
    ...
    if to_int(old.x) ~= to_int(new.x) then
        return true
    end
    if to_int(old.y) ~= to_int(new.y) then
        return true
    end
    return false
end
...
function showPausedText()
    if canvasElementExists("pausedrect") then
        return
    end
    if canvasSize.w >= 100 then
...
function drawEscText()
    if canvasElementExists(floater, "esctxt") then
        return
    end
    if canvasElementExists(floater, "crosshair") then
        if canvasSize.w >= 100 then
...
function canvasElementExists(canvas, id)
    if not (canvas and id) then
        return false
    end
    if getmetatable(canvas).__type ~= "hs.canvas" then
        return false
    end
...
function create_floater()
    cur_pos = hs.mouse.getAbsolutePosition()

    -- make sure we have a valid window to track
    if not w then
        w = hs.window.focusedWindow()
        if not w then
            return createFail("no focused window")
        end
        if w == hs.window.desktop() then
            return createFail("can't float the desktop window")
        end
        if not w:isVisible() then
            createFail("can't float an invisible window")
        end
        i = w:id()
        if not i then
            return createFail("couldn't obtain the windowID")
        end
    ...
    floater_origin = {
        x = valid_origin(
            floater_center.x - canvasAdj.x,
            win_scr_frame.x,
            win_scr_frame.x + win_scr_frame.w - canvasSize.w
        ),
        y = valid_origin(
            floater_center.y - canvasAdj.y,
            win_scr_frame.y,
            win_scr_frame.y + win_scr_frame.h - canvasSize.h
        )
    }
    ...
    if floater then
        ...
        floater:clickActivating(false):mouseCallback(
            function(_c, _m, _i, _x, _y) --
                --[[
        _c = canvas
        _m = mouse action
        _i = element id
                            ...
                            while _cMouseAction do
                                ...
                                if mouseHasMoved(last_pos, cur_pos) then
                                    ...
                                    if _buttons.right and not _updatesPaused then -- move capture area
                                        local delta = {
                                            x = (to_int(cur_pos.x - starting_pos.x)),
                                            y = (to_int(cur_pos.y - starting_pos.y))
                                        }
                                        ...
                                        _updateCaptureOrigin(
                                            {
                                                x = valid_capture_pos(
                                                    orig_capture_origin.x + delta.x,
                                                    win_frame.w - canvasSize.w
                                                ),
                                                y = valid_capture_pos(
                                                    orig_capture_origin.y + delta.y,
                                                    win_frame.h - canvasSize.h
                                                )
                                            },
                                            canvasAdj
                                        )
