...
function obj:toggleWebview()
    print("toggling" .. "isShown: " .. tostring(obj.isShown) .. " running: " .. tostring(obj.running))
    ...
    if obj.isShown == false then
        obj.plexWebview:show():bringToFront(true)
        obj.isShown = true
    elseif obj.isShown == true then
        obj.plexWebview:hide()
        obj.isShown = false
    end
end
...
function obj:init()
    ...
    self.plexInfoMenu = hs.menubar.new():setClickCallback(obj.toggleWebview):setTitle("ⓘ")
    self.plexControlMenu = hs.menubar.new():setClickCallback(obj.togglePlayPause):setIcon(iconPause)

    if self.plexMenu == nil then
        self.plexMenu = hs.menubar.new():setClickCallback(obj.toggleWebview):setIcon(icon, true)
    end
    ...
    for line in io.lines(script_path() .. "plex.js") do
        injectJqueryResult = injectJqueryResult .. line
    end
    ...
    self.plexJS:injectScript({source = localjsScript, mainFrame = false, injectionTime = "documentEnd"}):setCallback(
        function(message)
            ...
            elseif message.body.hasPlayer and message.body.hasPlayer == true then
                ...
                if obj.screenClass ~= "small" and obj.showProgressBar and message.body.podcast.episodeTitle then
                    ...
                    obj.menubarCanvas =
                        hs.canvas.new({x = 0, y = 0, h = menubarHeight, w = 250}):appendElements(
                        {
                            id = "songProgress",
                            type = "rectangle",
                            action = "fill",
    ...
    self.plexWebview =
        hs.webview.newBrowser(self.rect, {developerExtrasEnabled = true}, self.plexJS):url(plexWeb):allowTextEntry(true):level(
        3
    ):shadow(true):windowCallback(
        function(action, webview, state)
